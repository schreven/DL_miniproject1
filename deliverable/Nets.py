...
class Net2(nn.Module):
    def __init__(self, size, chn_conv, ker_conv, ker_pool, nb_hidden, len_IN_lin):
        ...
        self.fc3 = nn.Linear(nb_hidden[1], 2)
...
class Net3(nn.Module):
    ...
    def forward(self, x):
        ...
        x = F.leaky_relu(F.avg_pool1d(x, kernel_size=self.ker_pool[0]))
        x = self.dropout(x)
        x = F.leaky_relu(self.conv2(x))
        x = F.leaky_relu(F.max_pool1d(x, kernel_size=self.ker_pool[1]))
        x = self.dropout(x)
        x = F.leaky_relu(self.conv3(x))
        x = F.leaky_relu(F.max_pool1d(x, kernel_size=self.ker_pool[2]))
        #x = self.dropout(x)
        x = F.leaky_relu(self.fc1(x.view(-1,self.len_IN_lin)))
        x = self.dropout(x)
...
def train_model(model, criterion, optimizer_, train_input, train_target, nb_iters, mini_batch_size, use_tol = False, inter_states = False, test_input = 0, test_target = 0):
    eta = 1e-3
    err_array = np.zeros((int((nb_iters)/25),3))
    i_ = 0
    for e in range(0, nb_iters):
        sum_loss = 0
        optimizer = optimizer_(model.parameters(), lr = eta, weight_decay = 0.001)
        ...
        if e%25 ==0 and inter_states:
            ...
            i_ +=1
            if train_error <= 0.5 and use_tol ==True:
                print('tolerance attained at iteration {}'.format(e))
                err_array[i_:] = [[e, train_error,test_error],]*int(((nb_iters-e)/25)-1)
                break
            print('iter {:.02f}: train_error {:.02f}; test_error {:.02f}, loss:{:.02f}'.format(e, train_error,test_error,sum_loss))
      #  if e==300:
       #     eta = 10**-(3.5)
       # if e==500:
       #     eta = 10**-(4)
